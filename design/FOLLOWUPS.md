...
- **Status:** `landed (NO-BUMP)` on `main`; **a PATCH release `mk-codec 0.4.1` + crates.io publish is required at toolkit P6** before the toolkit can consume the accessor (toolkit currently pins `mk-codec 0.4.0`). **Tier:** `feature` (cross-repo companion).
