...
- **Status:** `wont-fix — moot per SPEC §2.4 (SingleString unreachable for v0.1 conforming KeyCards).` Closed during v0.1.1 Phase 1 Task 1.3 (`design/MILESTONE_v0_1_1.md`). The smallest valid v0.1 bytecode is 80 bytes (1+1+4+1+73), already above SINGLE_STRING_LONG_BYTES = 56; the SingleString branch in `encode_with_chunk_set_id` is dead code under the v0.1 wire format, and the `chunk_set_id` argument is therefore never silently dropped under any conforming input.
