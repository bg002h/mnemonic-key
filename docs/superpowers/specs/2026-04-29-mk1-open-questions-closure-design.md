...
**(d) Naming alignment:** Renamed the 20-bit per-encoding random tag from md1's "wallet identifier" to **`chunk_set_id`** across both repos. The original name conflicted with `Policy ID` and `Wallet Instance ID` and meant neither of those things; it identifies "all chunks belonging to this card-encoding," nothing more. Wire format unchanged; documentation/code rename only. **Resolution:** shipped in [md-codec v0.9.0](https://github.com/bg002h/descriptor-mnemonic/releases/tag/md-codec-v0.9.0) as docs-and-symbols-only (~85 sites). See `design/FOLLOWUPS.md::chunk-set-id-rename` for the closing pin.
...
**(1) `chunk_set_id` rename — sequencing pin (resolved).** md-codec v0.8.x originally used "wallet identifier" for the 20-bit per-encoding random tag in the chunked-header; the rename to `chunk_set_id` disambiguated from `Policy ID` and `Wallet Instance ID`. Wire format unchanged; documentation and code-symbol rename only.
...
**(3) BIP-draft cross-reference completeness (resolved).** mk1's BIP draft must cross-reference: BIP 93 (codex32 plumbing reuse), BIP 32 (xpub serialization), BIP 380 (origin notation), BIP 388 (wallet policy and template framing for Policy ID semantics), and the published md1 BIP (linkage protocol, shared-parser conventions, `chunk_set_id` field). Resolved 2026-04-29 — see [`design/AUDIT_bip_cross_reference_completeness.md`](../../design/AUDIT_bip_cross_reference_completeness.md). 74 cross-references audited; 9 drifts found and fixed inline. The "post-rename of `wallet identifier` → `chunk_set_id` in md1" precondition is met (md-codec v0.9.0 shipped 2026-04-29).
