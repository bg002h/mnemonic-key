# Verifying reproducibility of the `mk` musl release binaries

> **Scope.** This document covers the **x86_64-unknown-linux-musl** and
> **aarch64-unknown-linux-musl** `mk` CLI binaries published on each
> `mk-cli-v*` release. It is the per-binary verify recipe for mnemonic-key's
...
> ~15-line caller stub (`.github/workflows/musl-binaries.yml` `repro` job), the
> re-homed `musl-binaries` build, and `Cross.toml`.
>
> **`mk` is fork-free.** Unlike the toolkit, `mk` does NOT depend on the
> miniscript `[patch.crates-io]` git fork. Its transitive `md-codec` dependency
> is the **crates.io** release (not a git fork), and `miniscript` arrives via the
> crates.io `13.0.0`. Its `cargo vendor` graph is therefore a clean crates.io
> graph, so the `[source]` activation is the **TWO-block** form (crates-io +
> vendored-sources) — there is **no git-fork `[source]` stanza** here.
...
```sh
git clone https://github.com/bg002h/mnemonic-key
cd mnemonic-key
git checkout <mk-cli-vX.Y.Z>   # the release tag
...
CONTAINER_IMAGE=ghcr.io/bg002h/repro-musl-mnemonic-key@sha256:<BUILT-DIGEST>
docker pull "$CONTAINER_IMAGE"      # no auth needed — the package is PUBLIC
```

> **Maintainer one-time setup — the `repro-musl-mnemonic-key` GHCR package MUST be Public.**
...
> Packages → `repro-musl-mnemonic-key` → Package settings → Danger Zone → Change visibility →
> Public.
...
docker run --rm --network=none \
  ...
  bash -euxo pipefail -c '
    ...
    cargo build --locked --offline --release \
      --target x86_64-unknown-linux-musl -p mk-cli --bin mk \
      --config '"'"'source.crates-io.replace-with="vendored-sources"'"'"' \
      --config '"'"'source.vendored-sources.directory="vendor"'"'"'
    tar --sort=name --owner=0 --group=0 --numeric-owner --mtime="@$SOURCE_DATE_EPOCH" \
      -cf - -C target/x86_64-unknown-linux-musl/release mk \
      | gzip -n -9 > mk-<VER>-x86_64-linux-musl.tar.gz
  '
```
...
- **`CFLAGS` / `CFLAGS_<triple>` `-ffile-prefix-map`** strips absolute paths from
  the libsecp256k1 objects compiled by `cc-rs` under `musl-gcc` (mk's only C dep,
  via `secp256k1-sys` ← `secp256k1` 0.29.1).
...
- **The TWO-block job-scoped `[source]` activation is mandatory.** The `vendor/`
  ...
  since 1.63). Because `mk` is **fork-free**, only the two `cargo vendor`-emitted
...
container — see `.github/workflows/musl-binaries.yml` `musl-binaries` x86_64 leg).

## 5. Expected per-artifact SHA-256 + the provenance tuple

Each release attaches, per arch:

- `mk-<VER>-<arch>-linux-musl.tar.gz` — the static musl binary tarball.
- `SHA256SUMS.<arch>` — its SHA-256.
- `PROVENANCE.<arch>.txt` — the tuple. For x86_64:

  ```
  artifact:          mk-<VER>-x86_64-linux-musl.tar.gz
  sha256:            <hash>
  ...
  ```
  artifact:          mk-<VER>-aarch64-linux-musl.tar.gz
  sha256:            <hash>
  source_commit:     <full 40-char SHA>
  source_date_epoch: <epoch>
...
|---|---|---|
| Linux x86_64 / aarch64, static musl (`mk-<ver>-<arch>-linux-musl.tar.gz`) | `SHA256SUMS.x86_64` / `SHA256SUMS.aarch64` | `SHA256SUMS.x86_64.minisig` / `SHA256SUMS.aarch64.minisig` |
...
repo's `musl-binaries.yml` `repro` job) does not just rebuild once — it builds
`mk` at **two distinct** real paths (`/build-a/src` and `/build-b/src`, both
remapped to `/build`) and asserts the two binaries + tarballs + libsecp `.o` are
...
`musl-binaries.yml` has a `workflow_dispatch` trigger; a bare "Run workflow"
click runs the `repro` caller job (the toolkit gate) against the current branch.
On a `workflow_dispatch` event `github.ref` is the branch, not a `mk-cli-v` tag,
so the tag-gated `musl-binaries` build + release-upload job is skipped — only the
gate runs. That proves `mk`'s offline two-block reproducibility BEFORE any
release tag relies on it.
...
ci/repro/gzip-residue.sh mk-<VER>-x86_64-linux-musl.tar.gz 03
```

(The `MINISCRIPT_REV` env the gate scripts read is **empty** for `mk` — that
...
cross build --locked --offline --release \
  --target aarch64-unknown-linux-musl -p mk-cli --bin mk \
  --config 'source.crates-io.replace-with="vendored-sources"' \
  --config 'source.vendored-sources.directory="vendor"'

tar --sort=name --owner=0 --group=0 --numeric-owner --mtime="@$SOURCE_DATE_EPOCH" \
  -cf - -C target/aarch64-unknown-linux-musl/release mk \
  | gzip -n -9 > mk-<VER>-aarch64-linux-musl.tar.gz
```

The same TWO-block `[source]` activation (mk is fork-free), the same `--locked
